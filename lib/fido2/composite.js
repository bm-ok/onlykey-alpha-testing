...
// Both halves answer with a 32-byte shared secret: X25519_SS_SIZE and
// MLKEM_SS_SIZE are both 32 (okpqc.cpp). Stating it lets pollForResponse apply
// the chunk-shape check instead of returning on the first reply of any length.
const COMPOSITE_SS_LEN = 32;

// 57 (the device's HID packet size) * 4, leaving room for the 4-byte 0xFF
// header inside the 245 bytes a keyhandle can carry. Same value and same
// reasoning as onlykey-pgp.js's u2fSignBuffer().
const MAX_SEND_CHUNK = 228;
...
async function compositeDecrypt(fido2, channel, sharedSecret, slot, data, {
    ...
    // Digits are computed over the WHOLE plaintext payload, not per chunk - the
    // device hashes the reassembled packet_buffer once the final packet lands.
    const digits = challengeDigitsForPayload(payload);

    // One keyhandle carries at most 245 bytes (255 minus the 10-byte header),
    // so an ML-KEM-768 ciphertext at 1088 bytes has to go as several requests.
    // MAX_SEND_CHUNK matches onlykey-pgp.js's u2fSignBuffer() exactly: 4 x the
    // device's 57-byte packet size, leaving room for the 4-byte 0xFF header.
    //
    // opt2 marks the FINAL packet - without it the device keeps waiting for
    // more and never primes the challenge. opt3 is the packet number and must
    // strictly increase: ok_extension.cpp drops any packet whose opt3 is <= the
    // high-water mark, which is how it survives Windows 10 1903 sending every
    // FIDO2 request twice. Each chunk is encrypted separately under the transit
    // key, as the browser does.
    //
    // No delay between chunks. The production loop sleeps a second per packet,
    // but each ctaphidViaWebauthn() is a full round trip that only resolves
    // once the device has answered, which is the acknowledgement a timer was
    // standing in for.
    let primed = null;
    for (let off = 0, packetnum = 1; off < payload.length; off += MAX_SEND_CHUNK, packetnum++) {
        const chunk = payload.slice(off, off + MAX_SEND_CHUNK);
        const isFinal = off + MAX_SEND_CHUNK >= payload.length;
        // eslint-disable-next-line no-await-in-loop
        primed = await ctaphidViaWebauthn(
            fido2, origin, OKDECRYPT, slot, isFinal ? 1 : 0, packetnum,
            encryptPayload(chunk, sharedSecret), timeoutMs
        );
    }

    await answerChallenge(channel, digits);
    const resp = await pollForResponse(fido2, {
        origin, timeoutMs, maxMs, expected: COMPOSITE_SS_LEN,
    });
