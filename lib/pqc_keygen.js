...
const { checkStatus, probeDeviceResponsive } = require('./device');
...
async function enterConfigModeConfirmed(channel, primaryPin, { attempts = 4, isfadeMarginMs = 2500 } = {}) {
    for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        try {
            // Wait for the device itself to say it is done, instead of
            // guessing at the ~2-3s it spends busy after the long-press.
            // The DEBUG handler runs inline in loop(), so the echo cannot
            // come back until that work has finished.
            await probeDeviceResponsive(channel);

            // Require a positive 'locked' reading: config mode's re-auth step
            // forces unlocked=false, so passing through 'locked' is what
            // proves the press registered as a long press rather than being
            // silently ignored. This is a reliable question only because
            // checkStatus() now reads with a window wider than the firmware's
            // 1 Hz INITIALIZED broadcast (see check_status.py and QUIRKS.md);
            // with the old 100ms read a locked device reported 'unknown' ~86%
            // of the time, and asserting on 'locked' passed only by luck.
            const status = await checkStatus({ retries: 2, retryDelayMs: 600 });
...
// Convenience wrapper for the common "just get the device into config mode"
// case (test/08, test/12, and others previously duplicated this exact
// sequence locally): opens its own SEREMU channel, unlocks/confirms,
// long-presses button 6 via enterConfigModeConfirmed() above, then closes
// the channel - config mode is a device-side flag that persists across
// separate CLI processes/connections until the device relocks, so nothing
// needs to keep holding this channel open afterward.
async function enterConfigMode({ primaryPin = PINS.primary } = {}) {
