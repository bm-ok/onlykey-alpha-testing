...
// if re-unlock doesn't confirm within the timeout - the isfade race means
// the long-press can silently no-op, in which case the "re-unlock" digits
// just get read as normal (still-unlocked) slot-button presses and never
// confirm via checkStatus(). `isfadeMarginMs` is a blind wait - there's no
// reliable debug print for "isfade cleared" to key off (see unlockOnce's
// comment on DEBUG-serial output dropping under load), and the window
// it's approximating (~2s after unlock's fadeon()) was confirmed live:
// 500ms here reliably lost the long-press (every following digit got read
// as a slot action, never a PIN guess), same failure every time.
