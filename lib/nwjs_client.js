...
// Destroys every open page tab and opens a fresh one on `url`.
//
// This is the only recovery available for Chromium's native WebAuthn dialog
// ("Something went wrong / The request timed out"). That dialog is browser
// chrome: it is not in the DOM, CDP cannot click it, and this machine has no
// input-injection tool (no xdotool/python-xlib/pyautogui - checked). It also
// blocks the window's device calls indefinitely while a human is expected to
// press Close. Since the dialog is owned by its window, destroying the window
// destroys the dialog - which is something CDP *can* do, via the same plain
// HTTP /json/close/<id> that closeExtraPages() already uses.
//
// Deliberately reopens on a neutral url by default: reopening straight onto an
// app page would immediately re-run doSetTime() -> OKCONNECT -> FIDO2 and can
// simply raise the same dialog again, which is how a recovery turns into a
// loop.
//
// ORDER IS LOad-BEARING: open the replacement first, close the old ones after.
// Closing the last remaining page tab does not leave nw running headless on
// its background page - it quits the whole app. Confirmed live 2026-08-01:
// a close-then-reopen version of this killed the nw process outright, leaving
// CDP dead on 9222 and three orphaned nwjs-sdk processes behind. Keeping at
// least one window alive at all times avoids that entirely.
async function recycleWindow({ url = 'about:blank', timeoutMs = 15000 } = {}) {
    const before = (await listTabs()).filter((t) => t.type === 'page');
    ...
    await evalOn(bg.webSocketDebuggerUrl, `return await openAppWindow(${JSON.stringify(url)});`);

    // Wait for the replacement to register as its own CDP target, rather than
    // guessing at a delay - closing the old windows before it exists would
    // momentarily leave none, which is the quit condition above.
    const deadline = Date.now() + timeoutMs;
    let fresh = null;
    while (Date.now() < deadline) {
        fresh = (await listTabs()).filter((t) => t.type === 'page').find((t) => !beforeIds.has(t.id));
        if (fresh) break;
        await new Promise((r) => setTimeout(r, 200));
    }
    if (!fresh) throw new Error(`openAppWindow(${url}) did not produce a new page tab within ${timeoutMs}ms`);

    for (const old of before) {
        await fetch(`${CDP_ROOT}/json/close/${old.id}`).catch(() => {});
    }
    return fresh;
}
...
module.exports = { listTabs, isAlive, evalInPage, evalInBackground, getAppPage, getConsoleLog, recycleWindow, CDP_ROOT };
