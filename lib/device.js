...
class OnlyKeyDevice {
    ...
    // The confirm is only sent once the device has actually acknowledged the
    // arm - the firmware sets wipeConfirmPending and prints the prompt in the
    // same branch (okcore.cpp ~2482-2498), so a 'C' sent before that print
    // lands with nothing pending and is silently treated as a cancel.
    async wipeDevice({ full = false, timeoutMs = 5000 } = {}) {
        this.channel.clearBuffer();
        this.pressButton(full ? '9' : '0', { long: true });
        const armed = full ? /'9' full wipe requested/ : /'0' userspace wipe requested/;
        try {
            await this.channel.waitFor(armed, timeoutMs);
        } catch (e) {
            throw new Error(`Device never armed the ${full ? 'full' : 'userspace'} wipe: ${e.message}`);
        }
    ...
    //
    // okcore.cpp's DEBUG serial handler (~2475-2480) prints
    // "I received from DEBUG: <digit-code>" from the *terminator* branch -
    // i.e. once per complete press (digit byte + '\n'/' '), for every DEBUG
    // command, not just PIN digits. That makes it the one universal
    // acknowledgement available on this channel, and the firmware dispatches
    // the press inline in loop() straight afterwards, so the next press
    // cannot be echoed until this one's handler has returned.
    //
    ...
    // byte arrived. This is the fallback for presses that have no such print.
    async pressButtonAcked(digit, { long = false, timeoutMs = 5000 } = {}) {
...
// Byte 90 ('Z'): not a command in the DEBUG serial handler. okcore.cpp
// (~2504-2543) dispatches only '0'/'9'/'8'/'C' and the digits '1'-'6';
// everything else falls through having done nothing except clear a pending
// wipe confirmation, of which there is none during a readiness probe.
...
async function probeDeviceResponsive(channel, { timeoutMs = 8000 } = {}) {
    ...
    channel.send([PROBE_BYTE, SHORT_PRESS]);
