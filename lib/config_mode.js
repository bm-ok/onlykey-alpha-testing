...
// The restart is a DEBUG-only '8' long-press over the same SEREMU channel the
// unlock needs anyway - a reboot drops the underlying HID handle regardless of
// who opened it, so the channel is closed and reconnected around it either way.
// Sends the DEBUG restart press and confirms the device ACTED on it, by
// watching it leave the USB bus.
//
// The press used to be fired blind. Presses are dropped rather than queued
// while the firmware is busy (QUIRKS.md), so a dropped one left the device
// running and unlocked, and the only symptom was
// "Expected 'locked' after restart, got 'unlocked'" - a true statement about a
// press that was never received.
//
// The obvious acknowledgement, the firmware's `I received from DEBUG: <code>`
// echo, does NOT work for this particular press. It is printed before the
// handler runs, but the handler is CPU_RESTART(), which tears the USB stack
...
async function sendRestartAcked(channel, { attempts = 3, absenceMs = 4000, pollMs = 50 } = {}) {
    for (let i = 0; i < attempts; i++) {
        ...
        // Still enumerated and running: the press was dropped, so send another.
    }
    throw new Error(
        `device never left the USB bus after ${attempts} restart presses - the presses are being dropped`
    );
}
