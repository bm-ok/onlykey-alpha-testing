...
// The firmware's DEBUG line protocol (okcore.cpp, touch_sense_loop). Return is
// the only terminator; the '!' suffixes select a press-duration tier, and the
// firmware refuses a line longer than DBG_LINE_MAX.
// LINE_MAX matches DBG_LINE_MAX, which is one SEREMU OUT report (32 bytes) -
// so anything that fits in a single write also fits the firmware's parser.
const LINE_END = 0x0a;
const LINE_MAX = 32;
const HOLD = {
    tap: '',        // duration 1
    hold: '!',      // 128 - the >=72 actions
    long: '!!',     // 200 - also >=140 and >=180
    longest: '!!!', // 400 - also >=360
};
...
class SeremuChannel {
    ...
    // so a whole line can be sent together here.
    send(bytes) {
    ...
    // One line of the firmware's DEBUG protocol (okcore.cpp, touch_sense_loop).
    // Return is the only terminator and the firmware zeroes its buffer after
    // it, so every call here is a complete, independent command.
    sendLine(text) {
        this.send([...Buffer.from(text, 'latin1'), LINE_END]);
    }

    // One or more button presses as a single line. The firmware replays them
    // one per loop() iteration, so a whole PIN goes in one write and there is
    // no host-side inter-press delay to get wrong.
    //
    // `hold` picks the duration tier - see HOLD. The tiers exist because the
    // firmware's payload() has more bands than a single "long press" can reach
    // (>=72 hold actions, >=140 key labels, >=180 and >=360 on DUO).
    sendPress(buttons, { hold = 'tap' } = {}) {
        if (!(hold in HOLD)) {
            throw new Error(`hold must be one of ${Object.keys(HOLD).join(', ')}, got ${hold}`);
        }
        const line = String(buttons).split('').map((b) => {
            if (b < '1' || b > '6') throw new Error(`button must be 1-6, got '${b}'`);
            return `${b}${HOLD[hold]}`;
        }).join('');
        if (line.length > LINE_MAX) {
            throw new Error(`press line is ${line.length} bytes, firmware accepts ${LINE_MAX}: ${line}`);
        }
        this.sendLine(line);
    }
...
module.exports = { SeremuChannel, findSeremuDevice, isOnlyKeyPresent, sleep, HOLD };
