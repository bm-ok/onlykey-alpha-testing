...
function findHID(hid_interface) {
    var hids = nodeHID.devices().filter((a)=>{ if(a.manufacturer == "CRYPTOTRUST") return a})
...
var looping = false;
var offsetInterface = 0;
setInterval(function () {
    if (looping) return;
    looping = true;
    try {
        loadInterface(3 + offsetInterface);
    }
    catch (e) {
        console.log(e);
    }
    try {
        loadInterface(2 + offsetInterface);
    }
    catch (e) {
        console.log(e);
    }
