...
`./arduino-1.6.5-r5-teensy_127` (firmware), `./0c-coder-python-onlykey` (CLI +
age plugin — **not** `./python-onlykey`, an earlier/stale clone kept around by
accident; see note below), `./lib-agent` (agent packaging),
`./onlykey.github.io` (web app), `./OnlyKey-App` (setup GUI), `./serial` (raw
HID capture tool this suite's `lib/hid.js` supersedes for scripted use).

**Fork correction (2026-07-23):** this suite was originally built and TC-04
was debugged against `./python-onlykey`, which turned out to be the wrong
clone — an older fork lacking the composite-PGP-PQC and derived-X-Wing work
that `OnlyKey-PQC-Test-Report.md`'s header table (`68c1c84`) actually
describes. The correct fork was re-cloned as `./0c-coder-python-onlykey`;
`okpqc-venv`'s editable `onlykey` install now points there instead. See the
TC-04 entry below for what did/didn't carry over.
