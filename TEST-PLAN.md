...
## Composite decrypt: a response wiped by its own staging call — found and fixed 2026-08-01

TC-11 step 3. Two faults, one on each side, and the firmware one is the kind
worth remembering: it produced a response of exactly the right length in which
every byte was wrong.

**Firmware.** `okpqc_decrypt()` writes each half's 32-byte shared secret into
`large_resp_buffer` and then calls `send_transport_response()` on it, so
`store_FIDO_response()` (`fido2/device.cpp`) received `data == large_resp_buffer`.
Its unencrypted branch did
...
— clearing the buffer *before* copying from it, so the copy moved zeros onto
zeros. Measured: a correct X25519 shared secret arrived at the host as **32
zero bytes**, with no error anywhere. Host and device agreed on the transit
key, the challenge was accepted, the reply was the right length; only the
content was wrong. Fixed by clearing just the region outside the payload, for
`data` anywhere inside the buffer rather than only at its start. Not
PQC-specific — it applies to any caller that stages into `large_resp_buffer`
and hands it back.

**Host.** `compositeDecrypt()` sent its payload as a single keyhandle, which
caps at 245 bytes, so a 1088-byte ML-KEM-768 ciphertext threw `Max size
exceeded` before reaching the device. Now chunked at 228 bytes (4 × the
device's 57-byte packet), matching `onlykey-pgp.js`'s `u2fSignBuffer()`: `opt2`
marks the final packet, without which the device waits for more and never
primes the challenge; `opt3` is a strictly-increasing packet number, without
which `ok_extension.cpp`'s duplicate suppression drops the early chunks
silently. The production loop's one-second-per-packet sleep is not reproduced —
each round trip resolves only once the device has answered, which is the
acknowledgement the timer stood in for.

**Worth noting about the two directions.** Signing's hard case is a large
*response* (3309 B out, chunked retrieval); decryption's is a large *request*
(1088 B in, chunked send). They exercise opposite ends of the transport and
were never going to fail the same way — which is why proving one said nothing
about the other.

## The browser half of TC-11 — what was actually wrong, 2026-08-01

Node proving the four device primitives did NOT make the browser work, and the
gap is worth recording because only one of the three faults was in the device
path at all.

**1. `composite_decrypt()` asked for an unknown length.** It called
`poll_for_response(null)`, so the loop returned the first binary reply of any
size and skipped the chunk-shape check entirely — the exact hole through which
a short or off-cursor reply gets accepted as a shared secret. Both halves
answer 32 bytes; the browser now says so, and carries the same chunk-shape
guard as the Node copy.

**2. Nothing reported progress, and the operations are long.**
`prime_composite()` sends an ML-KEM-768 ciphertext as five WebAuthn ceremonies,
and `poll_for_response()` then waits ~10 s while the device computes an
ML-DSA-65 signature — the whole time showing one unchanging status string. That
is indistinguishable from a hang, for a user as much as for a test, and
anything with a no-progress budget shorter than the operation abandons a
healthy device partway. Measured: a GUI decrypt gave up after 5 s with the
device still receiving chunks and no final packet yet sent. Fixed as a product
change — per-packet, per-poll and per-chunk status from `onlykey-3rd-party.js`,
mirrored by `pgp-pqc.js` into the status line of whichever operation is running.

**3. Two spec waits that could never have passed.** The challenge wait was a
flat 5 s from when it started, but it has to span the entire multi-keyhandle
send, since the device primes only once the final packet lands — so it expired
mid-send with the device visibly healthy and printing `OKDECRYPT Chunk` for
every packet. It is bounded by SILENCE now: any device output re-arms it, which
keeps a wedged device caught in seconds without capping a legitimately long
operation. Separately, the verify read broke on the first non-empty status,
and the click handler sets `"Verifying..."` synchronously — so it always
captured that transient string and reported it as the verdict. The assertion
failed with `expected a valid signature, got: Verifying...` while the page
displayed `Signature VALID` throughout.

**The lesson worth keeping.** Two of the three were the test misreporting a
working system, not the system failing. Both produced failure text that named
the wrong thing — a challenge that was never entered, and a signature that was
